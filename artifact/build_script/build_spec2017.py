...
def run(spec_path):
    ...
    if os.path.exists('%s/install.sh'%(src)):
        ...
        cmd = 'docker run --rm -v "%s:/spec2017_image" -v "$(pwd)/src:/data" suri_artifact:v1.0 sh -c "chmod -R +x /spec2006_image && /spec2017_image/install.sh -d /data/spec_cpu2017 -f"'%(src)
        print(cmd)
        os.system(cmd)
