...
def run(spec_path):
    ...
    if os.path.exists('%s/install.sh'%(src)):
        ...
        cmd = 'docker run --rm -v "%s:/spec2006_image" -v "$(pwd)/src:/data" suri:v1.0 sh -c "/spec2006_image/install.sh -d /data/spec_cpu2006 -f"'%(src)
        print(cmd)
        os.system(cmd)
        ...
        for opt in ['-O0', '-O1', '-O2', '-O3', '-Os', '-Ofast']:
            for lopt in ['bfd', 'gold']: 
                cmd = 'docker run --rm -v "$(pwd)/src:/data" -v "$(pwd)/script:/script" -v %s:/output suri:v1.0 sh -c "/script/build-spec2006.sh %s %s" '%(output, opt, lopt)
                print(cmd)
                os.system(cmd)
    else:
